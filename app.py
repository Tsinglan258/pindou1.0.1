...
TRANSLATIONS = {
    "ja": {
        ...
        "palette_label": "🎨 カラーセット",
        "palette_artkal": "Artkal 1024色",
        "palette_mard": "MARD 221色",
...
# -------------------------------------------------
# カラーセット定義（Artkal 1024色 / MARD 221色）
# path のJSONは artkal_color.json と完全同一構造
# ({"colors": [{"code","name","r","g","b"}, ...]}) であることが前提。
# 拡張ポイント: 新しいセットを追加する場合はここに1行足すだけでよい。
# -------------------------------------------------
PALETTE_OPTIONS = {
    "artkal1024": {
        "label_key": "palette_artkal",
        "path": os.path.join(BASE_DIR, "artkal_color.json"),
        "max_colors_cap": 1024,
    },
    "mard221": {
        "label_key": "palette_mard",
        "path": os.path.join(BASE_DIR, "morandi_221_color.json"),
        "max_colors_cap": 221,
    },
}
...
with st.sidebar:
    ...
    # --- カラーセット切り替え（Artkal 1024色 / MARD 221色） ---
    # 選択を切り替えると、次の変換実行時に該当JSONを自動読み込みする
    # （アプリの再起動は不要。read_agentへ渡すpathを変えるだけ）。
    palette_keys = list(PALETTE_OPTIONS.keys())
    palette_labels = [t(PALETTE_OPTIONS[k]["label_key"]) for k in palette_keys]
    default_palette_idx = palette_keys.index(st.session_state.get("palette_key", "artkal1024"))
    selected_label = st.selectbox(t("palette_label"), palette_labels, index=default_palette_idx)
    selected_palette_key = palette_keys[palette_labels.index(selected_label)]
    st.session_state["palette_key"] = selected_palette_key

    palette_info = PALETTE_OPTIONS[selected_palette_key]
    COLOR_DB_PATH = palette_info["path"]
    palette_max_colors = palette_info["max_colors_cap"]
    ...
    max_colors = None
    if use_max_colors:
        # カラーセットの総色数に応じて上限値を自動調整
        # （マッチングロジック・処理速度自体は変更しない。数値の入力範囲のみ調整）
        default_max = min(20, palette_max_colors)
        max_colors = st.number_input(
            t("max_colors_label"), min_value=1, max_value=palette_max_colors,
            value=default_max, step=1
        )
